...
const char* set_keyboard[12] = { "F%", "CTRL", "", "", "", "", "", "", "", "Back", "", "" };
...
int T2Input::get_keycode(int x, int y) {//from virtual keyboard
	if (y < screen_h - key_h * 5)
		return 255;
...
void T2Input::show_current_pressed_key() {
	...
	if (current_key >= VM_KEY_NUM1 && current_key <= VM_KEY_NUM9)
		...
		break;
		case VM_KEY_STAR:
			vm_graphic_fill_rect(scr_buf, 0, screen_h - key_h, key_w, key_h, fill_color, fill_color);
			break;
		case VM_KEY_NUM0:
			vm_graphic_fill_rect(scr_buf, key_w, screen_h - key_h, key_w, key_h, fill_color, fill_color);
			break;
		case VM_KEY_POUND:
			vm_graphic_fill_rect(scr_buf, key_w * 2, screen_h - key_h, key_w, key_h, fill_color, fill_color);
			break;
		}
}

void T2Input::handle_penevt(int event, int x, int y) {
	switch (event) {
...
void T2Input::handle_keyevt(int event, int keycode) {
...
void T2Input::draw_xy_char(int x, int y, const char* str) {
	...
	for (int i = 0; i < char_height; ++i) {
		unsigned short* scr_buf = (unsigned short*)this->scr_buf + x + (y + i) * screen_w;
...
void T2Input::draw_xy_str(int x, int y, const char* str) {
	...
	for (int k = 0; str[k]; ++k) {
		...
		for (int i = 0; i < char_height; ++i) {
			unsigned short* scr_buf = (unsigned short*)this->scr_buf + x + k * char_width + (y + i) * screen_w;
			for (int j = 0; j < char_width; ++j)
				if (((*font_ch) >> j) & 1)
					scr_buf[j] = textcolor;
			++font_ch;
...
void T2Input::draw() {
	...
	if (draw_kb) { // draw screen keyboard
		...
		for (int i = 0; i < 5; ++i)
			vm_graphic_line(scr_buf, 0, screen_h - (i + 1) * key_h, screen_w, screen_h - (i + 1) * key_h, 0xFFFF);

		vm_graphic_line(scr_buf, key_w, keyboard_h, key_w, screen_h, 0xFFFF);
		vm_graphic_line(scr_buf, key_w * 2, keyboard_h, key_w * 2, screen_h, 0xFFFF);

		vm_graphic_line(scr_buf, 0, keyboard_h + key_h / 2, key_w, keyboard_h + key_h / 2, 0xFFFF);
		vm_graphic_line(scr_buf, key_w * 2, keyboard_h + key_h / 2, screen_w, keyboard_h + key_h / 2, 0xFFFF);
		...
		draw_xy_str(key_w + key_w / 2 - strlen("Send") * char_width / 2, keyboard_h + (key_h - char_height) / 2, "Send");
		...
		case MAIN:
			...
			draw_xy_str(key_w / 2 - strlen("SET") * char_width / 2, screen_h - key_h + (key_h - char_height) / 2, "SET");
			draw_xy_str(key_w * 2 + key_w / 2 - 3 * char_width / 2, screen_h - key_h + (key_h - char_height) / 2, imput_modes[(int)cur_input_mode]);
	...
	if (!(time - last_input_time >= 1000 || last_input_time > time)) { //draw input mode
		int y = (input_cursor_y == 0 ? char_height : 0), x = screen_w - 3 * char_width;
		draw_xy_str_color(x, y, 0xFFFF, gray_color, imput_modes[(int)cur_input_mode]);
		//vm_graphic_fill_rect(scr_buf, 0, screen_h - key_h, key_w, key_h, gray_color, gray_color);
	}
	if (!draw_kb) {
		...
		case SECOND_CLICK:
			...
			if (x > screen_w - h)
				x = screen_w - h;
			if (screen_h - (input_cursor_y + char_height) > h)
		...
		case CTRL:
			...
			if (x > screen_w - h)
				x = screen_w - h;
			if (screen_h - (input_cursor_y + 1) * char_height > h)
